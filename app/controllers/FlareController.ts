...
class FlareController {
      ...
      try {
        ...
        setTimeout(async () => {
          const roundExecution = async () => {
            try {
              console.log(networkName, "🌼 Started getting initial info 🌼");
  ...
  enableAutoClaim = async (req: Request, res: Response) => {
    ...
    try {
      ...
      await AutoClaimModel.findOneAndUpdate({ address: address }, {}, { new: true, upsert: true });
    } catch (err) {
      console.log(err.message);
    }
  };
    ...
    try {
      ...
      for (let user of users) {
        ...
        if (isRight) {
          ...
          await AutoClaimModel.deleteOne({ address: user.address });
        }
      }
